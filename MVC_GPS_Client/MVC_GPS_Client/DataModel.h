...
enum FATALSINCLIENT { WSAStartupError, FailConnect, FailMessageExchange }; //это идёт к пользователю
enum MESSAGESFROMSERVER { ERR, GOODCONNECT, GOODAUTH, ENDCONNECT, ACKNOWLEDGE }; //Типы сообщений, приходящих с сервера
enum MESSAGETYPES { SERVICE, LOG, RMCMSG }; //Это идёт на сервер
...
enum FATALERROSINSERVER { LOGLIMIT, SUDDENOUT, NOAPPROVE, FORMERROR }; //это идёт на сервер
